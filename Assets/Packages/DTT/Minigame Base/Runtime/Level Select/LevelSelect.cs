...
using DTT.MinigameBase.Tweening;
using UnityEngine;
...
namespace DTT.MinigameBase.LevelSelect
{
    ...
    public class LevelSelect : MonoBehaviour
    {
        ...
        public void Populate(LevelDatabase database)
        {
            ...
            _pagination.Populate(Mathf.CeilToInt(_database.Data.Count / (float)15), 15);
            DisplayPageLevels(_pagination.CurrentPage);
        }
        
        /// <summary>
        /// Loads in the page at the start.
        /// </summary>
        ...
        private void AddLevels()
        {
            for (int i = 0; i < _pagination.ItemsPerPage; i++)
            {
                ...
                DTTween.Value(0, 1, 0.3f, i * 0.05f, Easing.EASE_OUT_BACK,
                    onValueChanged: (float val) => _levelButtonInstances[_i].transform.localScale = Vector3.one * val,
                    onComplete: () =>
                    {
